...
//***************************************** Traitement ********************************************
    ...
    // 1- Verification ADMIN
        if(!internauteEstConnecteEtEstAdmin()){
            header('location:../connexion.php');    // Si membre pas ADMIN, alors on le redirige vers la paeg de connexion qui est a la racine du site (en dehors du dossier admin).
        exit();
        }
    ...
    // 7- Suppression d'un produit:
        if(isset($_GET['action']) && $_GET['action'] == 'suppression' && isset($_GET['id_produit'])){
            ...
            if(!empty($produit_a_supprimer['photo']) && file_exists($chemin_photo_a_supprimer)){
                // si il y a un chemin de photo en base ET que le fichier existe (file_exists est une fonction predefinie), on peut le supprimer:
                unlink($chemin_photo_a_supprimer);  // Supprime le fichier specifié.
            }
    ...
    // 4- Enregistrement du produit en BDD.
        if($_POST){  // Equivalent a !empty($_POST) car si le $_POST est rempli, il vaut TRUE = formulaire posté
            // Ici il faudrait mettre les controles sur le formulaire.
            $photo_bdd ='';  // La photo subit un traitement spécifique en BDD. Cette variable contiendra sont chemin d'acces.

    // 9- Modification de la photo (suite):
        if(isset($_GET['action'])  && $_GET['action'] == 'modification'){
            // Si je suis en modification, je mets en BDD la photo du champ hidden photo_actuel du formulaire:
                $photo_bdd = $_POST['photo_actuel'];
        }

    // 5- Traitement de la photo.
        echo '<pre>'; print_r($_FILES); echo '</pre>';
        if(!empty($_FILES['photo']['name'])){   // Si une image a été uploader, $_FILES est remplie.

            // On constitue un nom unique pour le fichier photo.
                $nom_photo = $_POST['reference'] . '_' . $_FILES['photo']['name'];

            // On constitue le chemin de la photo enregisté en BDD:
            ...
            // Enregistrement du fichier photo sur le serveur:
                copy($_FILES['photo']['tmp_name'], $photo_dossier);  // On copie le fichier temporaire de la photo stocké au chemin indiqué par $_FILES['photo']['tmp_name'] dans le chemin $photo_dossier de notre serveur.
        }
            ...
            $contenu .= '<table class="table">';
                        ...
                        $contenu .= '<tr>';
                            for($i =0; $i < $resultat->columnCount(); $i++){
                                ...
                                $contenu .= '<th>' . $colonne['name'] .'</th>';  // getColumnMeta() retourne un array contenant notament l'indice "name" contenant le nom de la colonne.
                            }
...
// 3- Formulaire HTML
    ...
    // 8- Formulaire de modification avec présaisie des infos dans le formulaire:
        if(isset($_GET['id_produit'])){
            // Pour préremplir le formulaire, on requete en BDD les infos du produits passé dans l'URL
    ...
    ?>
        ...
        <form method="post" enctype="multipart/form-data" action=""> <!-- "multipart/form-data" permet d'uploader un fichier et de générer une superglobale $_FILES -->
            ...
            <input type="hidden" id="id_produit" name="id_produit" value="<?php echo $produit_actuel ['id_produit'] ?? 0; ?>"> <!-- champ caché qui receptionne l'id_produit necessaire lors de la modification d'un produit  existant -->
            ...
            <input type="file" id="photo" name="photo"><br><br>  <!-- Couplez avec l'attribut enctype="multipart/form-data" de la balise <form>, le type 'file'  permet d'uploader un fichier (ici une photo) -->
        ...
        <!--9 Modification de la photo-->
            <?php
                ...
                // Afficher la photo actuelle:
                    echo '<img src="'. $produit_actuel['photo'] .'" width="90" height="90"><br><br>';
                // Mettre le chemin de la photo dans un champs caché pour l'enregistré en base.
                    echo '<input type="hidden" name="photo_actuel" value="'. $produit_actuel['photo'].' ">';    // Ce champ renseigne le $_POST['photo_actuel'] qui va en BDD quand on soumet le formulaire de modification. Si on ne remplit pas le formulaire ici, le champ photo de la base est remplacer par un vide, ce qui efface le chemin de la photo.
                }
